...
export function renderNameTeams(root: HTMLElement) {
  ...
  button.addEventListener('click',()=>{sync();if(names.length<2)return;const count=Math.min(Number(teamCount.value),names.length);const groups=Array.from({length:count},()=>[] as string[]);shuffled(names).forEach((name,index)=>groups[index%count].push(name));generated.innerHTML=groups.map((group,index)=>`<article style="--team:${['#a974ff','#38a0ff','#ff625b','#ffd84f','#67e8c3','#ff8ed4'][index]}"><span>Team ${index+1}</span><ol>${group.map(name=>`<li>${escapeHtml(name)}</li>`).join('')}</ol></article>`).join('');page.announcement.textContent=`Created ${count} teams.`})
  renderEditor()
}
